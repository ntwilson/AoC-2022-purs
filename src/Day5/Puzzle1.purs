...
type Stack = { id :: Int, crates :: List Char }
...
parseInput input = do
  ...
  parseStack s = liftEither $ lmap parseErrorMessage $ runParser s do
    Parsing.skipSpaces
    crates <- List.many Parsing.letter
...
evaluateInstruction stacks { nCrates, destStack, sourceStack } = do
  ...
  {newDest, newSource} <- Array.foldM moveCrate {newDest:dest, newSource:source} (1 .. nCrates)
  ...
  where 
  moveCrate {newDest: dest, newSource: source} _ = do
    {head: crateToMove, tail: sourceCrates} <- List.uncons source.crates # noteM (i"Stack "source.id" ran out of crates!" :: String)
    let 
      newDest = dest { crates = crateToMove : dest.crates }
...
ans { instructions, stacks } = do
  ...
  pure (stacks # Array.mapMaybe (_.crates >>> List.head) # CUString.fromCharArray)
