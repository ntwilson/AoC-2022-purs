...
parseInput input = {map, startingLocation:_} <$> startingLocation
  ...
  startingLocation = 
    Array.findMap (Array.find (\{z} -> z == 'E')) map
      # noteM "Unable to find starting position 'E'"
  
isMoveValid :: Position -> Position -> Boolean
isMoveValid {z:location} {z:target} 
  | location == 'E' = (target == 'z' || target == 'y')
  | otherwise = location <= target || pred location == Just target
...
completedPath = List.find (NonEmptyList.head >>> _.z >>> (_ == 'a'))
